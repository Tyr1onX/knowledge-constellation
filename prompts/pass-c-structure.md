...
A Galaxy should normally contain several nodes when real evidence forms a cluster, but **one-node Galaxies are valid** when one accepted Knowledge Node represents a real, isolated experience/theme and no second node is justified. Never invent a companion node, merge unrelated Anchors, or create a synthetic umbrella solely to satisfy visual or schema shape. A small truthful Galaxy is better than a larger false one.
